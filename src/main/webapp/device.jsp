		...
		<div id="page-wrapper">
			<div id="page-inner">
				<div class="row">
					<div class="col-md-12">
						<h2>Ustawienia</h2>
						<form method="post" action="/device/${deviceId }"
						modelAttribute="device">
							<center>
								<div> Nazwa urządzenia</br> <input type="text" name="name" pattern="[0-9]{1-5}"
									value="${device.name }" /> </div>
									
								<div> Interwal pomiarow</br> <input type="text" name="configuration.measureInterval" pattern="[0-9]{1-5}"
									value="${device.configuration.measureInterval }" /> </div>
									
								<div> Interwal sprawdzania rozkazow</br> <input type="text" pattern="[0-9]{1-5}"
									name="configuration.ordersInterval" value="${device.configuration.ordersInterval }" />
								</div>
								<div></br></br>
								<table id="phonesTable">
									<c:forEach items="${device.configuration.authorizedNumbers}"
										var="phone" varStatus="status">
										<tr id="row_${status.index }">
											<td><input type="text"
												name="configuration.authorizedNumbers[${status.index }]" value="${phone }" /></td>
								...
								<input type="button" name="cancel" onClick="window.location='/devices'" value="Anuluj" /> </br> </br> 
